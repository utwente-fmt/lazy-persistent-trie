...
class LazyPartitionTpcc(warehouseCount : Int, warehouses : Array[Warehouse], items : MMap[Int, Item]) extends Tpcc {
  // Every buffer has a threadlocal array of snapshots, to avoid allocating space for snapshots on every transaction
  //val snapshot = new ThreadLocal[Array[Warehouse]]
  ...
  def new_order(w_id : Int, d_id : Int, c_id : Int, item_orders : Array[ItemOrder]) : Double = {
    ...
    val snapshots = scala.collection.mutable.Map[Int,Warehouse]()
    commit(diff.iterator, snapshots)
    ...
    //item_orders.foreach(io => snapshots(io.supply_w_id).stock.force(io.i_id))

    
    for((w,d) <- stockDiffs) {
      snapshots(w).stock.force(d)
    }
  ...
  def payment(w_id : Int, d_id : Int, c_w_id : Int, c_d_id : Int, c : Either[Int,String], amount : Double) : Unit = {
    ...
    snapshots(w_id).districts.get(d_id)
    snapshots(c_w_id).districts.get(c_d_id).customers.force(c_id)
  }
  ...
  def delivery(w_id : Int, o_carrier_id : Int) : Boolean = {    
    ...
    locks(w_id).synchronized {
      ...
      val next_districts = w.districts.map((d_id, d) => {
        ...
        if(first.isDefined) {
          ...
          val next_orders = d.orders.modify(o_id, o => {
            val next_order_lines = o.order_lines.eagerMap((_, ol) => ol.copy(delivery_d = ol_delivery_d))
      ...
      snapshot = w.copy(districts = next_districts)
      warehouses(w_id * 16) = snapshot
    }

    // force
    snapshot.districts.foreach((d_id, d) => {
    ...
    if(c.isRight) {
      ...
      district.customer_name_index.get(c.right.get) match {
        case Some(c_ids) => {
          ...
          buffer = buffer.sortBy(_._2.first)
  ...
  private def commit(it : Iterator[(Int, Warehouse => Warehouse)], snapshots : scala.collection.mutable.Map[Int,Warehouse]) : Unit = {         
    val (w_id, f) = it.next
    locks(w_id).synchronized {
      val nw = f(warehouses(w_id * 16))
      warehouses(w_id * 16) = nw
      snapshots(w_id) = nw
      if(it.hasNext) {
        commit(it, snapshots)
      }
...
object LazyPartitionTpcc {
  ...
  def apply(db : Database) = {
    ...
    new LazyPartitionTpcc(warehouseCount, warehouses, db.items)
  }
}