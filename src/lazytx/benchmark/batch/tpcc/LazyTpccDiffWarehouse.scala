...
class DatabaseDiffWarehouse(var optimistic : Database, update : MapDiff[Int,Warehouse], var forcef : Database => Database) extends Diff[Database] { 
  ...
  def new_order(w_id : Int, d_id : Int, c_id : Int, item_orders : Array[ItemOrder]) : Double = {
    ...
    for((w,sd) <- stockDiffs) {
      if(w != w_id)
        update.apply(w, w => w.copy(stock = w.stock.update(sd)))
    }
  ...
  def apply(db : Database) : Database = {
    Database(update.apply(db.warehouses), db.items)
  }
